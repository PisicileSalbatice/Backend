...
def create_exam_request(db: Session, request: schemas.ExamRequestCreate) -> models.ExamRequest:
...
def get_student_requests(db: Session, professor_id: int) -> List[models.ExamRequest]:
    return db.query(models.ExamRequest).filter(models.ExamRequest.professor_id == professor_id).all()

# Update the status of an exam request (e.g., approve or reject)
def update_exam_request_status(db: Session, request_id: int, status: str) -> Optional[models.ExamRequest]:
    db_request = db.query(models.ExamRequest).filter(models.ExamRequest.id == request_id).first()
    if db_request:
...
def update_user_settings(db: Session, user_id: int, settings: schemas.SettingsUpdate) -> dict:
    # Find user by ID
